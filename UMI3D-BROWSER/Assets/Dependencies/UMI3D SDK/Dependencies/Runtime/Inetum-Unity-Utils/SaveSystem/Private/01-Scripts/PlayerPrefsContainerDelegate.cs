...
{
    internal class PlayerPrefsContainerDelegate : IContainerDelegate
    {
        ...
        {
            string filePath = Path.Combine(directories, fileName);
            return PlayerPrefsManager.LoadFromFile(filePath, out content);
        }

        public bool WriteToJson(string content, string directories, string fileName)
        {
            string filePath = Path.Combine(directories, fileName);
            return PlayerPrefsManager.WriteToFile(filePath, content);
        }
    }
}